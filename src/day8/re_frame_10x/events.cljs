...
(rf/reg-event-fx
  ...
  [(rf/inject-cofx ::local-storage/load {:key "panel-width-ratio" :or 0.35})
   ...
   (rf/inject-cofx ::local-storage/load {:key "render-path-annotations?" :or false})
   rf/unwrap]
  (fn [{:keys [panel-width-ratio show-panel selected-tab filter-items app-db-json-ml-expansions
               ...
               app-db-follow-events? ambiance syntax-color-scheme categories render-path-annotations?]}
       {:keys [debug?]}]
    {:fx [(when using-trace?
          ...
          [:dispatch [::settings.events/render-path-annotations? render-path-annotations?]]
