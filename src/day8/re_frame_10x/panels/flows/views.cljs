(ns day8.re-frame-10x.panels.flows.views
  (:require-macros
  ...
  (:require
   ...
   [day8.re-frame-10x.panels.flows.events                       :as flows.events]
   [day8.re-frame-10x.panels.flows.subs                         :as flows.subs]
    ...
    [rc/button
     ...
     :label    [rc/h-box
                ...
                :children [[material/add
                            {:size styles/gs-19s}]
                           "path inspector"]]
     :on-click #(rf/dispatch [::flows.events/create-path open-new-inspectors?])]))
...
(defn data-path-annotations []
  (let [render-path-annotations? @(rf/subscribe [::flows.subs/data-path-annotations?])]
    [rc/h-box
     :align    :center
     :children [[rc/checkbox
                 :model     render-path-annotations?
                 :label     "data path annotations"
                 :on-change #(rf/dispatch [::flows.events/set-data-path-annotations? %])]
                [rc/gap-f :size styles/gs-7s]
                [rc/box
                 :attr  {:title "When ticked, you can right-click on the rendered data (below) to obtain path data \n and cause focus etc. But this feature comes with a performance hit on rendering which \n is proportional to the size/depth of flows. So, if your flows is large and you are \n noticing a delay/pause in rendering flows, untick this option to get better performance."}
...
(defn pod-header [{:keys [id path path-str open? diff? sort? editing? edit-str expand?]} data]
  (let [ambiance       @(rf/subscribe [::settings.subs/ambiance])
        ...
        refresh-editor #(rf/dispatch-sync [::flows.events/set-edit-str
                                           {:id       id
                                            :value    (serialize-special-types data)
                                            :refresh? true}])
        big-data?      (not @(rf/subscribe [::flows.subs/small-data? {:id id}]))]
    [rc/v-box
     :children
     [[rc/h-box
       ...
       [[pod-header-section
         ...
         [[rc/box
           ...
           :attr   {:title    (str (if open? "Close" "Open") " the pod bay doors, HAL")
                    :on-click (handler-fn (rf/dispatch [::flows.events/set-path-visibility id (not open?)]))}
        ...
        [rc/h-box
         ...
         [[rc/input-text
           ...
           :attr            {:on-blur #(rf/dispatch [::flows.events/update-path-blur id])}
           :width           "100%"
           :model           path-str
           ;;:on-change     (fn [input-string] (rf/dispatch [:flows/search-string input-string]))
           :on-change       #(rf/dispatch [::flows.events/update-path {:path-str % :id id}])
           :on-submit       #()                   ;; #(rf/dispatch [::flows.events/add-path %])
           :change-on-blur? false
           :placeholder     "enter an flows path like [:todos 1]"]]]

        (when (> (count path) 0)
          [buttons/icon
           {:icon     [material/clear]
            :title    "Clear path in current inspector"
            :on-click #(rf/dispatch [::flows.events/update-path {:id id :path ""}])}])
        ...
        (when (> (count path) 0)
          [buttons/icon
           {:icon     [material/arrow-drop-up]
            ...
            :on-click #(rf/dispatch [::flows.events/update-path
        ...
        [pod-header-section
         ...
         :justify  :center
         :align    :center
         :attr     {:on-click (handler-fn (rf/dispatch [::flows.events/set-diff-visibility id (not diff?)]))}
         :children
         [[rc/checkbox
           :model diff?
           :label ""
           #_#_:style {:margin-left "6px"
                       :margin-top  "1px"}
           :on-change #(rf/dispatch [::flows.events/set-diff-visibility id (not diff?)])]]]
        [pod-header-section
         :width    "49px"
         :justify  :center
         :align    :center
         :attr     {:on-click (handler-fn (rf/dispatch [::flows.events/set-sort-form? id (not sort?)]))}
         :children
         [[rc/checkbox
           :model sort?
           :label ""
           :on-change #(rf/dispatch [::flows.events/set-sort-form? id (not sort?)])]]]
        [pod-header-section
         :width    "49px"
         :justify  :center
         :align    :center
         ...
         [(when (or (not big-data?) expand?)
            [buttons/icon {:icon     [(if expand? material/unfold-less material/unfold-more)]
                           ...
                           :on-click #(rf/dispatch [::flows.events/expand {:id id}])}])]]
        [pod-header-section
         :width    styles/gs-50s
         :justify  :center
         :children
         [[buttons/icon
           {:icon     [material/close]
            ...
            :on-click #(rf/dispatch [::flows.events/remove-path id])}]]]
        [pod-header-section
         :width styles/gs-50s
         :justify :center
         :children
         [[buttons/icon {:icon     [(if editing? material/unfold-less material/edit)]
                         ...
                         :on-click (if editing?
                                     #(rf/dispatch [::flows.events/finish-edit id])
                                     #(do (rf/dispatch [::flows.events/start-edit id])
        ...
        [rc/v-box
         ...
         [[rc/h-box
           ...
           [[:input {:type "file"
                     ...
                     :on-change #(do (rf/dispatch [::flows.events/open-file
                                                   (first (.-files (.-target %)))
                                                   [::flows.events/set-edit-str
                                                    {:id id :refresh? true}]])
                                     (set! (.-value (.-target %)) ""))}]
            ...
            [buttons/icon
             {:icon [material/download]
              ...
              :label "Export"
              :on-click #(rf/dispatch [::flows.events/save-to-file
                                       (cond-> "re-frame-db" path (str "__" path) :do (str ".edn"))
                                       (serialize-special-types data)])}]
            [buttons/icon
             {:icon [material/refresh]
              :label "Refresh"
              :title "Synchronize with the current value in flows"
              :on-click refresh-editor}]
            [buttons/icon
             {:icon [material/check-circle-outline]
              :title "Set the value of flows to the editor value."
              :label "Set!"
              :on-click #(userland.re-frame/dispatch
                          [::flows.events/edit path edit-str])}]]]
          [:textarea
           {:default-value edit-str
            ...
            :key @(rf/subscribe [::flows.events/editor-key id])
            :on-change #(rf/dispatch [::flows.events/set-edit-str
...
(defn pod [{:keys [id path open? diff? sort? expand?] :as pod-info}]
  (let [ambiance     @(rf/subscribe [::settings.subs/ambiance])
        ...
        flows-after (rf/subscribe [::flows.subs/current-epoch-flows-after])
        data         @(rf/subscribe [::flows.subs/path-data {:id id}])]
    [rc/v-box
     :children
          ...
          [rc/v-box
           ...
           [[cljs-devtools/simple-render-with-path-annotations
             {:data         data
              ...
              :db           @flows-after}]]])
        (when render-diff?
          (let [flows-before (rf/subscribe [::flows.subs/current-epoch-flows-before])
                [diff-before diff-after _] (when render-diff?
                                             (clojure.data/diff (get-in @flows-before path)
                                                                (get-in @flows-after path)))]
            [rc/v-box
             :children
             [[rc/v-box
               :class    (styles/flows-inspector-link ambiance)
               :justify  :end
               :children
               [[rc/hyperlink-href
                 :label "ONLY BEFORE"
  ...
  [rc/h-box
   ...
   [[svgs/round-arrow]
    [rc/label
     :style {:width      "160px"
             :margin-top "22px"}
     :label "see the values in flows by adding one or more inspectors"]]])
...
(defn pod-section []
  (let [pods @(rf/subscribe [::flows.subs/paths])]
    [rc/v-box
     :size     "1"
     :children
     (into
