...
(defn ^string truncate
  ...
  (let [c (count string)]
    (if (> (count string) n)
      (str "…" (subs string (- c n) c))
      string)))

(defn ^:string truncate-kw
  [kw]
  (let [the-ns (namespace kw)]
    (str
      (when (keyword? kw) ":")
      (when the-ns
        (let [segments (str/split the-ns #"\.")]
          (str
            (when (> (count segments) 1) (str "…"))
            (last segments))))
      (when the-ns "/")
      (name kw))))
...
(defn preview-param
  ...
  ([depth enter-pred max-depth param]
   (cond
     (keyword? param) (truncate-kw param)
     (or (symbol? param) (string? param))
