(ns mranderson048.reagent.v0v8v0.reagent.ratom
  ...
  (:require [mranderson048.reagent.v0v8v0.reagent.debug :as d]))

(defmacro reaction [& body]
  `(mranderson048.reagent.v0v8v0.reagent.ratom/make-reaction
    (fn [] ~@body)))

(defmacro run!
  "Runs body immediately, and runs again whenever atoms deferenced in the body change. Body should side effect."
  [& body]
  `(let [co# (mranderson048.reagent.v0v8v0.reagent.ratom/make-reaction (fn [] ~@body)
                                         :auto-run true)]
     (deref co#)
     co#))
...
(defmacro with-let [bindings & body]
  ...
  (let [v (gensym "with-let")
        ...
        add-destroy (when destroy
                      `(let [destroy# ~destroy]
                         (if (mranderson048.reagent.v0v8v0.reagent.ratom/reactive?)
                           (when (nil? (.-destroy ~v))
                             (set! (.-destroy ~v) destroy#))
                           (destroy#))))
        asserting (if *assert* true false)]
    `(let [~v (mranderson048.reagent.v0v8v0.reagent.ratom/with-let-values ~k)]
       (when ~asserting
         (when-some [c# mranderson048.reagent.v0v8v0.reagent.ratom/*ratom-context*]
